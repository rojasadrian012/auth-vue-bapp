...
interface CbiTestData {
    questions: Question[];
}

const cbiTestData: CbiTestData = {
...
/*calculateScores: function (answers) {
        ...
        const scaleQuestions = {
            ...
            work_burnout: [7, 8, 9, 10, 11, 12, 13],
            client_burnout: [14, 15, 16, 17, 18, 19]
        };

        // Calcular cada escala
        Object.keys(scaleQuestions).forEach(scale => {
            ...
            scores[scale] = count > 0 ? Math.round(sum / count) : 0;
        });

        // Calcular puntuación total
        const totalQuestions = Object.keys(answers).length;
        const totalSum = Object.values(answers).reduce((a, b) => a + b, 0);
        scores.total = totalQuestions > 0 ? Math.round(totalSum / totalQuestions) : 0;
    ...
    interpretScore: function (score) {
        if (score >= 0 && score <= 49) {
            return { level: "Bajo", description: "Nivel bajo de burnout", color: "#4CAF50" };
        } else if (score >= 50 && score <= 74) {
            return { level: "Moderado", description: "Nivel moderado de burnout", color: "#FF9800" };
        } else if (score >= 75 && score <= 89) {
            return { level: "Alto", description: "Nivel alto de burnout", color: "#F44336" };
        } else if (score >= 90 && score <= 100) {
            return { level: "Muy Alto", description: "Nivel muy alto de burnout", color: "#D32F2F" };
        }
        return { level: "No determinado", description: "Puntuación no válida", color: "#9E9E9E" };
    }
*/
...
/*
import { cbiTestData } from './mockData/cbiTest.js';

export default {
  data() {
    return {
      testData: cbiTestData,
      answers: {},
      results: null
    };
  },
  methods: {
    submitTest() {
      this.results = this.testData.calculateScores(this.answers);
    }
  }
};
*/